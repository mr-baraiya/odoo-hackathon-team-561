/**
 * DealFlow360 — End-to-End Automation & Verification Script
 */

const path = require('path');
const { calculateBlendedRiskScore } = require(path.join(__dirname, '../backend/src/service/riskScoreEngine'));
const { calculateFulfillmentSplits } = require(path.join(__dirname, '../backend/src/service/fulfillmentEngine'));
const { generateHybridBillingSchedule, calculateMidCycleProration } = require(path.join(__dirname, '../backend/src/service/billingEngine'));
const { getUpsellSuggestions } = require(path.join(__dirname, '../backend/src/service/upsellEngine'));

console.log('====================================================');
console.log(' DealFlow360 — End-to-End Automation & Logic Test');
console.log('====================================================\n');

// STEP 1: Blended Risk Score Calculation
console.log('--- Step 1: Testing Blended Risk Score Calculation ---');
const riskTest = calculateBlendedRiskScore({
  customerTierCode: 'gold',
  lineItems: [
    { productId: '501', categoryType: 'hardware', categoryCeilingPct: 15, unitPrice: 2000, costPrice: 1200, quantity: 1, discountPct: 12 },
    { productId: '503', categoryType: 'service', categoryCeilingPct: 10, unitPrice: 1000, costPrice: 800, quantity: 1, discountPct: 18 },
  ],
  orderDiscountPct: 0,
});
...
const upsellTest = getUpsellSuggestions({
  currentCartLines: [{ productId: '501', quantity: 1, unitPrice: 4500, costPrice: 2700, discountPct: 10 }],
  availableProducts: [
    { id: '501', name: 'Server', base_price: 4500, cost_price: 2700, category_name: 'Hardware', is_active: true },
    { id: '503', name: 'Onsite Setup Service', base_price: 1200, cost_price: 950, category_name: 'Services', is_active: true },
    { id: '504', name: 'Cloud SaaS Subscription', base_price: 350, cost_price: 50, category_name: 'Subscriptions', is_promoted: true, is_active: true },
  ],
  upsellRules: [{ baseProductId: '501', suggestedProductId: '503', coPurchaseScore: 0.9, minMarginPctRequired: 10 }],
});
...
const warehouseTest = calculateFulfillmentSplits(
  [{ lineId: 'l1', productId: '501', productName: 'Server Rack', quantity: 7 }],
  [
    { id: '701', name: 'Main Central Warehouse', shippingCostWeight: 1.0, stockMap: { '501': 5 } },
    { id: '702', name: 'East Coast Depot', shippingCostWeight: 1.2, stockMap: { '501': 3 } },
  ]
);
...
if (warehouseTest.fulfillmentSplits.length === 2) {
...
const prorationTest = calculateMidCycleProration({ originalMonthlyPrice: 350, newMonthlyPrice: 500, daysInCycle: 30, daysRemaining: 18 });
