...
const Reports = () => {
  ...
  useEffect(() => {
    ...
    reportsApi.getSales({ status: statusMap[status] })
  ...
  const byStatus = Object.entries(records.reduce((groups, item) => {
  ...
  const byTeam = Object.entries(records.reduce((groups, item) => {
    const teamName = item.sales_rep_name || 'Unassigned';
    const group = groups[teamName] || { team: teamName, quotes: 0, revenue: 0, discountTotal: 0 };
    group.quotes += 1;
    ...
    ...group,
    avgDiscount: group.quotes ? (group.discountTotal / group.quotes).toFixed(1) : '0.0',
  }));
  ...
  const handleExportXLS = () => {
    ...
    const rows = report.records.map((item) => [item.quote_number || item.id, item.customer_name || '', item.status, item.total_amount || 0]);
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={formatCurrency(records.reduce((total, item) => total + Number(item.total_amount || 0), 0))}
          trend="+14.2%"
        />
        <StatCard
          title="Active Quotes"
          value={records.filter((item) => !['confirmed', 'rejected'].includes(item.status)).length}
          trend="+8"
        />
        <StatCard
