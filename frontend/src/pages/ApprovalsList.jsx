...
const ApprovalsList = () => {
  ...
  const columns = [
    ...
    {
      header: 'Stage',
      render: (row) => <span className="font-medium text-xs text-textmain">{row.stage}</span>
    },
    {
      header: 'Assigned To',
      render: (row) => <span className="text-xs text-textsub">{row.assignedTo}</span>
    },
    {
      header: 'Date',
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-textmain flex items-center gap-2">
            <CheckSquare className="w-5 h-5 text-primary" /> Approval Governance Queue
          </h1>
          <p className="text-xs text-textsub mt-0.5">Review pricing policy compliance and margin exception requests</p>
        </div>
      </div>
      ...
      <Card>
        ...
        <div className="flex border-b border-bordercolor mb-5">
          {['pending', 'returned', 'approved', 'all'].map((tab) => (
            ...
            >
              {tab === 'pending' ? `Pending (${approvals.filter(a => a.status === 'pending').length})` : tab}
            </button>
          ))}
        </div>
