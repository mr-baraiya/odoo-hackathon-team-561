...
const ApprovalDetail = () => {
  ...
  const fallbackApproval =
    approvals.find((a) => a.id === id || a.quoteId === id) || approvals[0];
  ...
  const approval = details
    ? {
        ...
        status:
          details.quote.status === "pending_approval"
            ? "pending"
  ...
  const handleApprove = async () => {
    try {
      const response = await approvalApi.approve(
        approval.id,
        "Approved from approval workspace",
      );
      toast.success(response.data.message || "Approval submitted");
      navigate("/approvals");
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Approval could not be submitted",
      );
    }
  };
  ...
  const handleConfirmReturn = async () => {
    try {
      const response = await approvalApi.returnForRevision(
        approval.id,
        reasonInput || "Needs business justification",
      );
  ...
  const handleConfirmReject = async () => {
    try {
      const response = await approvalApi.reject(
        approval.id,
        reasonInput || "Policy non-compliant",
      );
      toast.success(response.data.message || "Quotation rejected");
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Rejection could not be submitted",
      );
    }
    setModalType(null);
    navigate("/approvals");
  ...
  if (loading)
    return (
      <Layout>
        <div className="p-6 text-sm text-textsub">
    ...
    <Layout>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-textmain flex items-center gap-2">
              Approval Request:{" "}
              <span className="text-accent">{approval.quoteId}</span>
            </h1>
        ...
        <div className="space-y-4">
                ...
                {approval.violations && approval.violations.length > 0 ? (
                  approval.violations.map((v, i) => (
                    <tr key={i} className="hover:bg-hoverbg/40">
                      ...
                      <td className="py-3 px-3 text-center font-medium text-textsub">
                        {v.limit}%
                      </td>
          ...
          <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-lg text-xs space-y-1">
            ...
            <p className="text-amber-800">
              ...
              <span className="font-semibold">{approval.overallPattern}</span>
            </p>
          </div>
        </div>
      </Card>
      ...
      >
        <p className="text-xs text-textsub mb-3">
          Provide feedback to sales rep on what needs modification:
        </p>
        <textarea
          rows="4"
          value={reasonInput}
      ...
      >
        <p className="text-xs text-textsub mb-3">
          Please state the policy reason for rejecting this proposal:
        </p>
        <textarea
          rows="4"
          value={reasonInput}
