...
const InvoiceDetail = () => {
  ...
  const handleConfirmPayment = () => {
    ...
    alert(
      `Payment of ${formatCurrency(invoice.total)} recorded for Invoice ${invoice.id}!`,
    );
  };
  ...
  return (
    <Layout>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-textmain flex items-center gap-2">
              Invoice Statement:{" "}
              <span className="text-accent">{invoice.id}</span>
            </h1>
      ...
      <Card className="print-invoice print:p-0 print:border-none print:shadow-none print:rounded-none">
        {/* Invoice Top Header */}
        <div className="flex items-center justify-between pb-6 border-b border-bordercolor">
          <div>
            ...
            <p className="text-xs text-textsub mt-1">
              100 Tech Plaza, Suite 800, San Francisco, CA 94107
            </p>
        ...
        <div className="grid grid-cols-2 gap-6 py-6 border-b border-bordercolor">
          <div>
            <span className="text-xs font-semibold text-textsub uppercase tracking-wider block mb-1">
              Billed To:
            </span>
              ...
              {invoice.items &&
                invoice.items.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="py-3 px-3 text-center font-bold text-textmain">
                      {item.qty}
                    </td>
                    <td className="py-3 px-3 text-right font-medium text-textsub">
        ...
        <div className="space-y-4 text-xs">
          <div>
            ...
            >
              ...
              <option value="Credit Card">
                Corporate Credit Card (Visa/Mastercard)
              </option>
          ...
          <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-lg">
            <p className="font-bold text-emerald-900">
              Total Settlement Amount: {formatCurrency(invoice.total)}
            </p>
            <p className="text-emerald-700 text-[11px] mt-0.5">
              Recording this payment will immediately update Accounts Receivable
              balances.
            </p>
          </div>
        </div>
