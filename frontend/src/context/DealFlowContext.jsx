...
export function DealFlowProvider({ children }) {
  ...
  // Logic Calls
  const calculateRisk = async (customerTierCode, lineItems, orderDiscountPct = 0) => {
    const res = await axios.post(`${API_BASE}/quotations/calculate-risk`, {
      customerTierCode,
      lineItems,
      orderDiscountPct,
    });
  ...
  const getFulfillmentSplit = async (quoteId, overrideSplits = null) => {
    const res = await axios.post(`${API_BASE}/quotations/${quoteId}/fulfillment-split`, { overrideSplits });
    return res.data;
  };

  const fetchUpsellSuggestions = async (quoteId, cartLines = []) => {
    const res = await axios.post(`${API_BASE}/quotations/${quoteId}/upsell-suggestions`, { cartLines });
    return res.data;
  };
  ...
  const prorateChange = async (quoteId, params) => {
    const res = await axios.post(`${API_BASE}/quotations/${quoteId}/prorate-change`, params);
    return res.data;
  };

  const cancelSubscriptionLine = async (quoteId, params) => {
    const res = await axios.post(`${API_BASE}/quotations/${quoteId}/cancel-subscription`, params);
    await refreshAllData();
    return res.data;
  };
  ...
  const sendNudge = async (alertId, quotationId, note) => {
    const res = await axios.post(`${API_BASE}/analytics/nudge`, { alertId, quotationId, note });
    await refreshAllData();
    return res.data;
  };
