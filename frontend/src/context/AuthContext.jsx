...
export const AuthProvider = ({ children }) => {
  ...
  const createSession = (userData, customToken = null) => {
    ...
    const newSession = {
      user: {
        ...
        avatar:
          userData.avatar ||
          (userData.name || userData.full_name || "US")
            .split(" ")
            .map((n) => n[0])
            .join("")
            .toUpperCase()
      ...
      token: (customToken || `jwt_token_${Date.now()}`).replace(
        /^Bearer\s+/i,
        "",
      ),
      expiresAt,
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      if (response.data && response.data.token) {
        return createSession(
          response.data.user || { email: cleanEmail, role: response.data.role },
          response.data.token,
        );
      }
    } catch (apiErr) {
      console.warn(
        "Backend API server unreachable or login failed on server. Falling back to local seed accounts.",
        apiErr?.message,
      );
    }
    ...
    if (!found) {
      throw new Error(
        "No user found with this email address. Contact System Administrator for access.",
      );
    }

    if (found.password !== password) {
      throw new Error(
        "Incorrect password. Please verify your login credentials.",
      );
    }
  ...
  const createUser = async (userData) => {
    ...
    try {
      ...
      console.warn(
        "Backend API unreachable for createUser, saving locally.",
        apiErr?.message,
      );
    }
