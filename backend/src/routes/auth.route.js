...
router.post("/login", async (req, res) => {
  ...
  if (!magicToken) {
    ...
    try {
      ...
      const databaseUser = await db.queryOne(
        `SELECT id, full_name, email, phone_number, role, customer_id, is_active
         FROM users
      ...
      if (databaseUser) {
        ...
        return res.json({ token: `Bearer ${token}`, user: databaseUser });
      }
    } catch (error) {
      console.warn(
        "Database login unavailable; using seed authentication fallback.",
        error.message,
      );
    } finally {
      if (db) db.release();
  ...
  if (magicToken) {
    const customerUser = seed.USERS.find(
      (u) => u.magic_link_token === magicToken,
    );
    if (customerUser) {
      const customerInfo =
        seed.CUSTOMERS.find((c) => c.id === customerUser.customer_id) || {};
      const token = jwt.sign(
        {
          id: customerUser.id,
  ...
  if (isValid) {
    ...
    if (user.role === "customer") {
      customerInfo =
        seed.CUSTOMERS.find((c) => c.id === user.customer_id) || {};
    }
    const token = jwt.sign(
...
router.get("/me", authenticateJWT, (req, res) => {
  ...
  if (req.user.customer_id) {
    customerInfo =
      seed.CUSTOMERS.find((c) => c.id === req.user.customer_id) || null;
  }
...
router.post("/magic-link", (req, res) => {
  ...
  user.magic_link_expires_at = new Date(
...
router.post("/verify-magic-link", (req, res) => {
  ...
  if (!user)
    return res
      .status(400)
      .json({ message: "Invalid or expired magic link token." });
