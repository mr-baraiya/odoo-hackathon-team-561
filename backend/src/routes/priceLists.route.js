...
router.get('/price-lists', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    console.error('[API GET /api/price-lists] DB query failed:', err.message);
  }
...
router.post('/price-lists', authenticateJWT, authorizeRoles('admin', 'finance_ops'), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const createdList = await db.queryOne(`
        INSERT INTO price_lists (name, tier_id, currency_code, is_active)
        VALUES ($1, $2, $3, $4)
        RETURNING *
    ...
    return res.status(500).json({ message: 'Failed to create price list.' });
  }
});
  ...
  try {
    ...
    try {
      if (isUUID(id)) {
        ...
        const updatedList = await db.queryOne(`
          UPDATE price_lists
...
router.post('/price-lists/:id/items', authenticateJWT, authorizeRoles('admin', 'finance_ops'), async (req, res) => {
  ...
  return res.status(400).json({ message: 'Failed to add price list item' });
});
